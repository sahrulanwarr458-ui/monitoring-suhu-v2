...
function updateMotorTimerDisplay(seconds, cycleState) {
    ...
    if (timerEl && seconds !== undefined && !isNaN(seconds)) {
    ...
    if (motorCyclePhase && cycleState) {
        if (cycleState === "ON") {
            ...
            if (motorStatusText) {
                motorStatusText.innerHTML = "Motor sedang berjalan 10 detik";
            }
        } else {
            ...
            if (motorStatusText) {
                motorStatusText.innerHTML = "Motor menunggu siklus 1 hari";
            }
        }
    }
    ...
    if (progressBar && seconds > 0 && cycleState) {
        let totalDuration;
        ...
        let progress = ((totalDuration - seconds) / totalDuration) * 100;
...
function updateTime() {
    ...
    if (timeElement) {
        timeElement.innerHTML =
            `${dateString}<br><span style="font-size: 14px; color: #a0aec0;">${timeString}</span>`;
    }
}
...
client.on("message", (topic, message) => {
    ...
    switch(topic) {
        ...
        case "inkubator/heartbeat":
            ...
            if (heartbeatEl) {
                heartbeatEl.innerHTML = new Date().toLocaleTimeString('id-ID');
            }
            ...
            if (!isNaN(uptimeSec)) {
                ...
                if (uptimeEl) {
                    uptimeEl.innerHTML = `${hours}h ${minutes}m`;
                }
            }
            break;
        ...
        case "inkubator/motor_next":
            updateMotorTimerDisplay(
                parseInt(val),
                document.getElementById("motorCyclePhase")?.innerText.includes("ON") ? "ON" : "OFF"
            );
            break;
        ...
        case "inkubator/high":
            ...
            if (highEl && !isNaN(parseFloat(val))) {
